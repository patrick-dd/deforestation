...
costK = 0.0377              # multiplicative constant on cost function
sigma = 0.675			    # standard deviation on cost shock
...
baselines = linspace(
    ...
    mean_base_deforestation + std_base_deforestation, 10
)
...
output_table = []
output_table.append(['Policy', 'Transfer', 'Payments', 'Avoided defo',
                    'Cost', 'Net cost', '\\\\'])
output_table.append(['Deforestation tax', round(defo_transfer, 4), round(
...
output_table.append(['AD: optimal', round(defo_transfer, 4), round(mean(
    defo_instance.sample_payments) / 1e9, 4), round(mean(
        defo_instance.sample_avoidedDeforestation)/1e6, 4), round(mean(
            defo_instance.sample_costperco2), 4), round(mean(
                defo_instance.sample_netcostperco2), 4), '\\\\'])
...
output_table.append(['AD: historical', round(defo_transfer, 4), round(mean(
    defo_instance.sample_payments) / 1e9, 4), round(mean(
        defo_instance.sample_avoidedDeforestation)/1e6, 4), round(mean(
            defo_instance.sample_costperco2), 4), round(mean(
                defo_instance.sample_netcostperco2), 4), '\\\\'])
...
for rl in baselines:
    ...
    output_table.append([round(rl, 4), round(ad_transfer_neg, 4), round(mean(
        ad_instance_neg.sample_payments) / 1e9, 4), round(mean(
            ad_instance_neg.sample_avoidedDeforestation)/1e6, 4), round(mean(
                ad_instance_neg.sample_costperco2), 4)])
