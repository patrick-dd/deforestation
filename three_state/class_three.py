...
class deforestation_growth:
    ...
    def bellman(self):
        ...
        for s in self.state:
            ...
            v, x = self.nDmaximum(
                ...
                ((0, income), (self.minLand, self.maxLand))
                )
            vals.append(v)
            policyK.append(x[0])
            policyL.append(x[1])
    ...
    def transferFinder(self, valueBaseline):
        ...
        """
        initialIncome = self.initialState[0]
        maxIncome = max(self.grid1)
        lowerTransfer = 0
        upperTransfer = maxIncome - initialIncome
        transfer = (upperTransfer + lowerTransfer)/2.0
        H = lambda T: valueBaseline(self.initialState) - \
            self.newvaluefunction(
                self.initialState[0] + T, self.initialState[1],
                self.initialState[2]
            )
        print 'Baseline: ', valueBaseline(self.initialState)
        print 'Tax: ', self.newvaluefunction(self.initialState)
        if self.adPolicyFlag is 'defoTax':
            if H(0) < 0:
                print 'ERROR: Value function under tax is greater than value'
                print 'function without tax'
                self.transfer = 0
                return 0
            count = 0
            for i in range(50):
                print count
                print H(transfer), transfer
                count += 1
                if abs(H(transfer)) < self.tol:
                    self.transfer = transfer * self.paymentstodollars
                    break
                elif H(transfer) > 0:
                    # Transfer too low
                    ...
                    # Transfer too high
                    upperTransfer = transfer
                    transfer = (upperTransfer + lowerTransfer) / 2.0
        else:
            if 0 < H(0):
                print 'ERROR: Value function under AD policy is lower than'
                print 'value function without tax'
                self.transfer = 0
                return 0
            count = 0
            for i in range(50):
    ...
    def stateSequenceGenerator(self, sequenceLength, baselinePolicy):
        ...
        paymentsequence = []
        costshocksequence = self.dist_c(
            self.mu_c, self.sigma_c, sequenceLength
        )
        if self.adPolicyFlag is None:
            ...
            for t in range(sequenceLength):
                ...
                output = self.productionFunction(investment, nextLand) + \
                    (1-self.delta)*investment
                incomesequence_base.append(output)
                landsequence_base.append(nextLand)
        ...
        for t in range(sampleLength):
            ...
            if output_avoidedDeforestation[-1] == 0:
                ...
                if self.adPolicyFlag == 'adSub':
                    ...
                    output_costperco2.append(
                        self.transfer / output_avoidedDeforestation[-1]
                    )
